 ...
 * независимо от часового пояса браузера/сервера
 */
export class DateTimeZoneUtil {
  ...
  /**
   * Форматировать дату для отображения в указанном часовом поясе
   */
  static formatForDisplay(date: Date | number | string, timezone: string, locale: string = 'ru-RU'): string {
    ...
    return dateObj.toDate().toLocaleDateString(locale, options)
  }
}
